...
<section class="hero"><div><div class="eyebrow">Campus marketplace</div><h1>Buy smarter.<br><em>Sell beautifully.</em></h1><p>Discover useful products from independent sellers — buy them outright, or rent for a while.</p><div class="hero-actions"><a class="btn" href="<%=ctx%>#products">Explore products</a><a class="btn btn-ghost" href="<%=ctx%>/app/register">Become a seller</a></div></div><div class="hero-card"><div class="hero-icon">✦</div><b>One marketplace.</b><span>Buyer · Seller · Admin</span><small>Secure sessions · BCrypt · JDBC</small></div></section>
...
<article class="product-card"><a class="product-link" href="<%=ctx%>/app/product?id=<%=p.id()%>" target="_blank" rel="noopener"><div class="product-img"><img src="<%=imageSrc(p.imageUrl(),ctx)%>" alt="<%=esc(p.name())%>"></div></a><div class="product-body">
...
<a class="product-link" href="<%=ctx%>/app/product?id=<%=p.id()%>" target="_blank" rel="noopener"><h3><%=esc(p.name())%></h3></a><p><%=esc(p.description())%></p>
